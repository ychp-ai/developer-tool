...
import { toolGroups as menuGroups, toolByPath } from '../tool-registry/registry'
...
export function useToolNavigation() {
  ...
  const { favorites, setFavorites, recentTools, setRecentTools } = useToolHistory(location.pathname)

  const isToolActive = useCallback(
    (tool: ToolDefinition) => !tool.isExternal && location.pathname === tool.path,
    [location.pathname],
  )
  ...
  const [groupSelection, setGroupSelection] = useState<{ pathname: string; groups: Set<string> } | null>(null)
  ...
  const setOpenGroups = (groups: Set<string>) => setGroupSelection({ pathname: location.pathname, groups })
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (document.activeElement === searchInputRef.current && searchQuery && flattenedTools.length > 0) {
        if (e.key === 'ArrowDown') {
          e.preventDefault()
          setSelectedSearchIndex((prev) => (prev < flattenedTools.length - 1 ? prev + 1 : prev))
  ...
  const favoriteTools = menuGroups.flatMap((group) => group.tools.filter((tool) => favorites.has(tool.path)))
